...
public class BioGeneratorContainer extends Container {
    public final BioGenTileEntity tileEntity;
    private final IWorldPosCallable canInteractWithCallable;

    public BioGeneratorContainer(final int windowId, final PlayerInventory playerInventory, final PacketBuffer data) {
        this(windowId, playerInventory, getTileEntity(playerInventory, data));
    }

    public BioGeneratorContainer(final int windowId, final PlayerInventory playerInventory, final BioGenTileEntity tileEntity) {
        super(Register.BIO_GENERATOR_CONTAINER.get(), windowId);
        ...
        this.addSlot(new SlotItemHandler(tileEntity.inventory, BioGenTileEntity.FUEL_SLOT, 80, 35));
    ...
    private static BioGenTileEntity getTileEntity(final PlayerInventory playerInventory, final PacketBuffer data) {
        Objects.requireNonNull(playerInventory, "playerInventory cannot be null!");
        Objects.requireNonNull(data, "data cannot be null!");
        final TileEntity tileAtPos = playerInventory.player.world.getTileEntity(data.readBlockPos());
        if (tileAtPos instanceof BioGenTileEntity)
            return (BioGenTileEntity) tileAtPos;
    ...
    public boolean canInteractWith(@Nonnull final PlayerEntity player) {
        return isWithinUsableDistance(canInteractWithCallable, player, Register.BIO_GENERATOR.get());
    }
}
